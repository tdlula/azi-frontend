...
export default function ChartPromptsManager({ onPromptSelect, showStats = true }: ChartPromptsManagerProps) {
  ...
  const forceRefreshDashboard = async () => {
    try {
  const response = await fetch(`${import.meta.env.VITE_PROD_API_BASE_URL || import.meta.env.VITE_DEV_API_BASE_URL || 'http://129.151.191.161:5000'}/api/dashboard-data?force_refresh=true`);
      if (response.ok) {
